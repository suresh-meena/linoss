...
def parse_env_file(path: Path) -> dict[str, str]:
    ...
    for line_number, raw_line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        if not separator:
            raise RemoteConfigError(f"invalid env line {line_number} in {path}: {raw_line!r}")
        key = key.strip()
...
def resolve_machine_name(env: dict[str, str], requested: str | None) -> str:
    ...
    raise RemoteConfigError(f"select a machine with --machine; configured machines: {configured}")
...
def require_machine_field(env: dict[str, str], machine: str, field: str) -> str:
    ...
    if value is None or value == "":
        raise RemoteConfigError(f"missing required field {field} for machine {machine!r}")
    return value
...
def resolve_machine(env: dict[str, str], requested: str | None) -> RemoteMachine:
    ...
    try:
        ...
        raise RemoteConfigError(f"invalid port {port_text!r} for machine {name!r}") from exc
    ...
    if auth_text == "key":
        ...
        auth = "password"
        if not password:
            raise RemoteConfigError(f"missing PASSWORD for password-auth machine {name!r}")
    else:
        raise RemoteConfigError(f"unsupported auth mode {auth_text!r} for machine {name!r}")
...
def command_rsync(args: argparse.Namespace) -> int:
    ...
    if args.direction == "upload":
        ...
        if args.dest is not None:
            ...
            message = (
                f"machine {machine.name!r} has no WORKDIR; "
                "pass --dest explicitly for upload"
            )
            raise RemoteConfigError(
                message
            )
        ...
        if args.source is not None:
            ...
            message = (
                f"machine {machine.name!r} has no WORKDIR; "
                "pass --source explicitly for download"
            )
            raise RemoteConfigError(
                message
            )
...
def command_smoke(args: argparse.Namespace) -> int:
    ...
    remote_command = f"cd {shlex.quote(machine.workdir)} && {smoke}" if machine.workdir else smoke
    return run_command(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    shell_parser = subparsers.add_parser("shell", help="Open a remote shell or run a command")
    ...
    rsync_parser = subparsers.add_parser("rsync", help="Sync files to or from a remote machine")
    ...
    rsync_parser.add_argument("--delete", action="store_true", help="Pass --delete to rsync")
    rsync_parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    smoke_parser = subparsers.add_parser("smoke", help="Run a small remote sanity check")
