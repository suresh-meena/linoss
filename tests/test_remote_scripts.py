...
def _run_script(script_name: str, *args: str, env_file: Path) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
...
def test_remote_print_config_resolves_selected_machine(tmp_path: Path) -> None:
    env_file = _write_remote_env(tmp_path)
    result = _run_script("remote-print-config", "--machine", "scratch-box", env_file=env_file)
...
def test_remote_smoke_dry_run_uses_machine_workdir(tmp_path: Path) -> None:
    env_file = _write_remote_env(tmp_path)
    result = _run_script("remote-smoke", "--machine", "dgx1", "--dry-run", env_file=env_file)
    assert "alice@dgx.example.edu" in result.stdout
